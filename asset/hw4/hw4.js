...
var _slicedToArray = function () {function sliceIterator(arr, i) {
  ...
  try {for (
    ...
    !(_n = (_s = _i.next()).done);
    _n = true){_arr.push(_s.value);
    if (i && _arr.length === i) break;}} catch (err) {_d = true;_e = err;} finally {
      try {if (!_n && _i["return"]) _i["return"]();} finally {
        if (_d) throw _e;}}return _arr;}
        return function (arr, i) {if (Array.isArray(arr)){return arr;} else if (Symbol.iterator in Object(arr)) {return sliceIterator(arr, i);} else {throw new TypeError("Invalid attempt to destructure non-iterable instance");}};}();var SPAWN_RATE = [10, 30];
...
function getColorStr(_ref13, noteIndex){
  ...
  var lightness = scale(relNoteIndex, [10, 100]);
